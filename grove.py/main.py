...
mapa = {'alarmaoff' : 0, 'alarmaon': 0, 'puertaopen': 0}
	...
	def print_gesture(self):
		...
		if data==self.GES_RIGHT_FLAG:
			...
			if data == self.GES_FORWARD_FLAG:
				...
				print("Right")
				puertaabierta = "Veces abierta : " + str(mapa['puertaopen'])
				alarmaencendida = "Veces alarma encendida : " + str(mapa['alarmaon'])
				alarmaapagada = "Veces alarma apagada : " + str(mapa['alarmaoff'])
				
				mensaje = puertaabierta + '\n' + alarmaencendida + '\n' + alarmaapagada
				f = open("estadisticas.txt", "w")
				f.write(mensaje)
				f.close()

				print("Estadisticas exportadas")
			...
			if data == self.GES_FORWARD_FLAG:
				...
				time.sleep(1)
				lcd.setCursor(0, 0)
				lcd.write('Alarma encendida')
				mapa['alarmaon'] = mapa['alarmaon'] + 1;
			...
			GPIO.output(26, False)
			lcd.setCursor(0, 0)
    			lcd.write('Puerta cerrada')

			mapa['puertaopen'] = mapa['puertaopen'] + 1;
			...
			GPIO.output(24, False)
          		time.sleep(1)
			lcd.setCursor(0, 0)
			lcd.write('Alarma apagada') 
			
			mapa['alarmaoff'] = mapa['alarmaoff'] + 1; 			
		else:
